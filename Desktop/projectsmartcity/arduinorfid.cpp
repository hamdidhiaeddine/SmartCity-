...
void ArduinoRFID::readSerialData()
{
    ...
    // Variables statiques pour stocker température et humidité
    ...
    while (serialBuffer.contains('\n')) {
        ...
        if (!line.isEmpty()) {
            ...
            // Données de température (format: "Temperature = XX.XX °C")
            else if (line.contains("Temperature =") && line.contains("°C")) {
                // Extraire la température
                QString tempStr = line;
                tempStr = tempStr.remove("Temperature =").remove("°C").remove("Â").trimmed();
                bool ok = false;
            ...
            // Données d'humidité (format: "Humidite = XX.XX %")
            else if (line.contains("Humidite =") && line.contains("%")) {
                // Extraire l'humidité
                QString humStr = line;
                humStr = humStr.remove("Humidite =").remove("%").trimmed();
                bool ok = false;
                float hum = humStr.toFloat(&ok);
                
                if (ok) {
                    lastHum = hum;
                    // Émettre le signal avec température et humidité
                    emit temperatureReceived(lastTemp, lastHum);
                    qDebug() << "💧 Humidité:" << hum << "% | Émission signal temp:" << lastTemp << "°C";
                }
            }
