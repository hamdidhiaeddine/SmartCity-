        ...
        if (QFile::exists(resourceCandidate)) {
            ...
            if (!found) {
                ...
                if (tf.open(QIODevice::WriteOnly | QIODevice::Text)) {
                    QTextStream out(&tf);
                    // write conservative defaults for jardins (vd/jardin1..jardin4)
                    out << "Jardin 1:vd/jardin1.mp4\n";
                    out << "Jardin 2:vd/jardin2.mp4\n";
                    out << "Jardin 3:vd/jardin3.mp4\n";
                    out << "Jardin 4:vd/jardin4.mp4\n";
                    tf.close();
                           ...
                           const QSize &size)
{
    ...
    // Fond dégradé vert nature pour jardins
    QLinearGradient gradient(0, 0, w, h);
    gradient.setColorAt(0, QColor(34, 139, 34));    // Vert forêt
    gradient.setColorAt(1, QColor(60, 179, 113));   // Vert médium
    ...
    // Icône nature (feuille) en haut à gauche
    p.setBrush(QColor(255, 255, 255));
    int iconX = 40, iconY = 35;
    p.drawEllipse(iconX + 5, iconY + 5, 30, 30);

    // Nom (grand, blanc)
    QFont nameFont("Arial", qMax(28, w/25), QFont::Bold);
    p.setFont(nameFont);
    p.setPen(Qt::white);
    p.drawText(100, 70, doctorName);

    // Spécialité (sous le nom, couleur accent)
    QFont specFont("Arial", qMax(14, w/50), QFont::Normal);
    p.setFont(specFont);
    p.setPen(QColor(144, 238, 144));  // Vert clair
    QString spec = specialite.isEmpty() ? "Jardinier" : specialite;
    ...
    QString tel = telephone.isEmpty() ? "+216 XX XXX XXX" : telephone;
    ...
    p.drawText(QRect(20, h - 25, w - qrSize - 60, 20), Qt::AlignLeft | Qt::AlignVCenter, 
               QString("SmartCity Jardins • %1").arg(QDate::currentDate().toString("yyyy")));
                          ...
                          const QSize &pageSize)
{
    ...
    html += "  background: linear-gradient(135deg, #228B22 0%, #3CB371 100%); ";
    ...
    html += ".specialite { font-size: 12px; color: #90EE90; text-transform: uppercase; letter-spacing: 1px; }";
    html += ".cabinet { font-size: 14px; font-weight: 600; margin: 15px 0 10px 0; }";
    html += ".contact { font-size: 11px; color: #C8FFC8; line-height: 1.8; }";
    html += ".footer { position: absolute; bottom: 15px; left: 35px; font-size: 9px; color: rgba(255,255,255,0.6); }";
    html += "</style></head><body>";
    html += "<div class='card'>";
    html += "<div class='icon'>🌿</div>";
    html += QString("<div class='name'>%1</div>").arg(doctorName.toHtmlEscaped());
    ...
    html += "</div>";
    html += QString("<div class='footer'>SmartCity Jardins • %1</div>").arg(QDate::currentDate().toString("yyyy"));
    html += "</div>";
    html += "</body></html>";
