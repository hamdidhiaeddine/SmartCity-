...
VideoDialog::VideoDialog(QWidget *parent)
    : QDialog(parent)
{
    setWindowTitle("Affichage vidéos");
    resize(1000, 700);
...
void VideoDialog::setupUi()
{
    ...
    // top: grid of cabinet buttons
    QWidget *topWidget = new QWidget(this);
    auto *grid = new QGridLayout(topWidget);

    QStringList names = m_metier.cabinetNames();
    if (names.isEmpty()) {
        names = QStringList{"cabinet1","cabinet2","cabinet3","cabinet4"};
    }
    ...
    QPushButton *addDoc = new QPushButton("Générer Carte de Visite", this);
...
{
    // Demander le nom du docteur
    QString docName = QInputDialog::getText(this, "Carte de Visite", "Nom du Docteur :");
    if (docName.trimmed().isEmpty())
        return;

    // Demander la spécialité
    QString specialite = QInputDialog::getText(this, "Carte de Visite", "Spécialité :");
    if (specialite.trimmed().isEmpty())
        specialite = "Médecin Généraliste";

    // Choisir cabinet
    QStringList names = m_metier.cabinetNames();
    QString cabinet = names.isEmpty() ? QString("Cabinet Médical") : names.first();
    if (names.size() > 1) {
        bool okCab;
        cabinet = QInputDialog::getItem(this, "Carte de Visite", "Cabinet :", names, 0, false, &okCab);
    ...
    if (email.trimmed().isEmpty())
        email = QString("dr.%1@cabinet.tn").arg(docName.simplified().replace(' ', '.').toLower());
    ...
    QDir outdir(QDir::current().filePath("cartes_visite"));
    if (!outdir.exists()) outdir.mkpath(".");

    QString sanitizedName = docName.simplified().replace(' ', '_');

    if (choice.startsWith("Image")) {
        QString out = outdir.filePath(QString("carte_visite_%1.png").arg(sanitizedName));
        if (m_metier.addDoctorCard(docName, specialite, cabinet, email, telephone, adresse, out)) {
            ...
            // fallback to PDF
            QString outPdf = outdir.filePath(QString("carte_visite_%1.pdf").arg(sanitizedName));
