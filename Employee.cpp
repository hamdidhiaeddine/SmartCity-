...
QVector<Employee> Employee::fetchAll(QString &errorText)
{
    ...
    q.prepare("SELECT \"ID_EMPLOYE\", \"NOM\", \"PRENOM\", \"POSTE\", \"SALAIRE\", \"ADRESSE\", \"TELEPHONE\", \"EMAIL\", \"ID_RES\" FROM \"SYSTEM\".\"EMPLOYES\" ORDER BY \"ID_EMPLOYE\" ASC");
    if (!q.exec()) { errorText = q.lastError().text(); return items; }
    while (q.next()) {
        Employee e;
        ...
        e.salaire = q.value(4).toDouble();
        e.adresse = q.value(5).toString();
        e.telephone = q.value(6).toLongLong();
        e.email = q.value(7).toString();
        e.idRes = q.value(8).toString();
        items.push_back(e);
    }
    return items;
}
