...
void MainWindow::chargerVehicules()
{
    ...
    while (query.next()) {
        ...
        ui->tableau_3->setItem(row, 0, new QTableWidgetItem(query.value("IMMAT").toString()));
        ...
        ui->tableau_3->setItem(row, 6, new QTableWidgetItem(query.value("DATE_MAINT").toString()));
        ++row;
    }
...
void MainWindow::onRFIDScanned(const QString &rfidCode)
{
    ...
    if (query.next()) {
        ...
        // Résident non trouvé
        QMessageBox::warning(this, "RFID - Accès Refusé", 
            QString("❌ ACCÈS REFUSÉ\n\n"
                    "Code RFID inconnu: %1\n\n"
